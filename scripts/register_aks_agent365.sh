...
OIDC_ISSUER=$(az aks show \
  ...
  --name "$CLUSTER" \
  --query "oidcIssuerProfile.issuerUrl" -o tsv 2>/dev/null || echo "")
...
UAMI_EXISTS=$(az identity show --resource-group "$RESOURCE_GROUP" --name "$UAMI_NAME" --query clientId -o tsv 2>/dev/null || echo "")
...
if [[ -n "$UAMI_EXISTS" ]]; then
  ...
  az identity create \
    ...
    --location "$LOCATION" \
    --only-show-errors
  UAMI_CLIENT_ID=$(az identity show --resource-group "$RESOURCE_GROUP" --name "$UAMI_NAME" --query clientId -o tsv)
fi

UAMI_PRINCIPAL_ID=$(az identity show --resource-group "$RESOURCE_GROUP" --name "$UAMI_NAME" --query principalId -o tsv)
UAMI_RESOURCE_ID=$(az identity show --resource-group "$RESOURCE_GROUP" --name "$UAMI_NAME" --query id -o tsv)
...
BLUEPRINT_APP_ID=$(az ad app list --display-name "$BLUEPRINT_NAME" --query "[0].appId" -o tsv 2>/dev/null || echo "")
...
echo "  ✓ Identifier URI: api://$BLUEPRINT_APP_ID"
echo "  ✓ API scope exposed: access_as_agent"
echo "  ✓ Tagged: M365Agent"
echo ""
...
AGENT_APP_ID=$(az ad app list --display-name "$AGENT_NAME" --query "[0].appId" -o tsv 2>/dev/null || echo "")
